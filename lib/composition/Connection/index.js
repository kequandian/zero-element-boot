...
import { AutoLayout } from "../../components"; // import AutoLayout from '../AutoLayout';

import Loading from "../../components/loading";

const promiseAjax = require("../../components/utils/request"); // import config from 'zero-element-boot\lib\components\config'
...
export default function Index(props) {
  ...
  useEffect(_ => {
    signWay(params.sign);
  }, [params]); // const [showSign,setSigncntent] = serState('')

  let layoutData = '';
  const layoutJsonPath = '';
  const localLayoutJson = layout; // 用于下载
  ...
  const onJarItemClick = item => {
    console.log(item, ' === item'); // document.body.scrollTop = document.documentElement.scrollTop = 0;
  ...
  function signWay(sign) {
    ...
    setmySign(sign);
    getconnection(sign); //签名文本框定时获取焦点
  ...
  const signcontent = m => {// signdata = m
    // setmySign(signdata)
    // console.log();
  }; //获取所有表
  ...
  function getconnection(sign) {
    ...
    promiseAjax(api).then(responseData => {
      if (responseData && responseData.code === 200) {
        ...
        setDetail(respData);
        setIsShowData(true);
        setmybutton(false); // console.log(respData);
      } else {
        setIsShowList(true);
        setIsShowData(false);
        alert('签名已过期!');
  ...
  function getRuler() {
    ...
    promiseAjax(api).then(responseData => {
      if (responseData && responseData.code === 200) {
        ...
        setIsShowData(true);
        setmybutton(true); // console.log(respData);
  ...
  function newRulerWay(ruler, rulercontent) {
    let api = `/dev/connection/snapshot/rulers/${ruler}?sign=${showmySign}`; //字符串转化成JSON
    // console.log(api);
    ...
    promiseAjax(api, rulerdata, {
      ...
      {
        if (responseData && responseData.code === 200) {
          let respdata = responseData.data; // console.log(respdata);
  ...
  function updataRulerWay(content, condata) {
    let api = `/dev/connection/snapshot/rulers/${content}?sign=${showmySign}`; //字符串转化成JSON
  ...
  function RulerDeployWay(content) {
    ...
    promiseAjax(api).then(responseData => {
      if (responseData && responseData.code === 200) {
        let respData = responseData.data; // respData = respData.replace(/[\'\\\\/\b\f\n\r\t]/g, '');
  ...
  function downSnapshot(content) {
    // console.log(content);
    printdataBaseSWay(content); // console.log(signdata);
  ...
  function getImages(content) {
    getImagesWay(content); //签名文本框定时获取焦点
  ...
  function getshow(content, isDataSheetDetail = false) {
    ...
    getshowWay(content); //签名文本框定时获取焦点

    SignText();
  }

  function getshowWay(item) {
    let api = `/dev/connection/json?sign=${showmySign}&pattern=${item}`; // let api = `/dev/connection/schema/json?pattern=${item}`;
  ...
  function getRulerContent(content) {
    settitle(content);
    getRulerContentWay(content); //签名文本框定时获取焦点
  }
