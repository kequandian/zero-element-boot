...
import React, { useState } from 'react'; // import useLayout from '@/components/hooks/useLayout';
// 无效果，仅传递数据
...
export default function OverlaySelector({
  ...
  const _isSelected = selected ? onSelected : isSelected; // 选中状态无需 default indicator,  两者相互切换,  默认状态可以不配置
  ...
  const SelectedIndicator = selectedIndicator === undefined || !_isSelected ? NextIndicator : selectedIndicator; // 选中状态，以及没有配置 overlay, hover 没有效果, overlay配置, 叠加 hover 效果
