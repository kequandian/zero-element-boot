...
import { ButtonGroup, Button, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, Input,
// 文本框
Stack,
// 布局组件 设置子元素坚决
FormControl,
...
FormLabel,
// label
FormErrorMessage, Spinner, useToast, Tabs, TabList, TabPanels, Tab, TabPanel, useTab, useMultiStyleConfig } from "@chakra-ui/react";
...
export default function ManageList(props) {
  ...
  //获取tabs列表数据
  function getTabsListData() {
    let newTabList = [];
    return promiseAjax(api, queryData).then(resp => {
      if (resp && resp.code === 200) {
        newTabList = resp.data.records;
        const addItem = {
          id: '-1'
        };
        newTabList.push(addItem);
        setNavCateListData(newTabList);
        setLoading(false);
      } else {
        console.error('获取列表数据失败 ==', resp);
      }
    }).finally(_ => {
      // fetchData(navListApi, {typeId: newTabList[0].id})
    });
  }
  ...
  //自定义tab按钮
  const CustomTab = /*#__PURE__*/React.forwardRef((props, ref) => {
  ...
  function TabsContent() {
    return tabsList && tabsList.length > 0 ? /*#__PURE__*/React.createElement(Tabs, {
      variant: "enclosed",
      style: {
        width: '900px'
      },
      defaultIndex: tabIndex
    }, /*#__PURE__*/React.createElement(TabList, null, tabsList.map((item, index) => {
      if (item.id === '-1') {
        return /*#__PURE__*/React.createElement(CustomTab, {
          key: `${index}_tab`,
    ...
    })), /*#__PURE__*/React.createElement(TabPanels, null, tabsList.map((item, index) => /*#__PURE__*/React.createElement(TabPanel, {
      key: `${index}_tabPanel`
    }, isLoading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Box, null, /*#__PURE__*/React.createElement(ListBody, null)))))) : null;
  }
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(ListBody, null), /*#__PURE__*/React.createElement(Modal, {
