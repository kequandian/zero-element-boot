...
export default function PreAutoLayout(props) {
  ...
  const testLayoutJsonData = testLayoutJsonObj && testLayoutJsonObj[0] || {}; //根据apiName 获取 API url
  ...
  const records = data && data.records; // 从layoutApi获取layoutJson
  ...
  return records && records.length > 0 && layoutJson && JSON.stringify(layoutJson) !== '{}' ? /*#__PURE__*/React.createElement(Box, {
    spacing: "3px"
