import React, { useState, useEffect } from 'react'; // import useTokenRequest from '@/components/hooks/useTokenRequest';
...
export default function index(props) {
  ...
  const [isLoading, setIsLoading] = useState(false); // const api = params.api || `/openapi/lc/apis/${params.apiName}`
  ...
  }, [params, api]); //通过 apiName 获取 api路径

  function getApiUrlByApiName() {
    promiseAjax(`/openapi/lc/apis/${params.apiName}`).then(res => {
      setIsLoading(false);
  ...
  function getJsonDataByLayoutName(api) {
    promiseAjax(api, params).then(res => {
      if (res && res.code === 200) {
        let data = res.data; // console.log('getJsonDataByLayoutName data = ', data)
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, urlNull ? /*#__PURE__*/React.createElement("div", {
    style: {
      margin: '10px'
    }
  }, errMsg) : isLoading ? /*#__PURE__*/React.createElement(Loading, {
    styles: {
      marginTop: '10px',
      marginLeft: '5px'
    }
  }) : jsonData && JSON.stringify(jsonData) !== '{}' && /*#__PURE__*/React.createElement(Tree, jsonData));
}