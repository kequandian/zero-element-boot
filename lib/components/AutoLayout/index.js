...
function AutoLayout({
  ...
  } = sugarLayout(layout) || {}; // const data = dataSource || rest || {}
  ...
  const _gateway = _layoutGateway ? typeof _layoutGateway === 'string' ? {
    xname: _layoutGateway
  } : sugarGateway(_layoutGateway) : undefined;

  const _NamedGateway = _layoutBinding || _gateway ? NamedGateway : NextIndicator; // handle container
  ...
  if (!layoutChildren && !container) {
    ...
    const _data_presenter = presenter || _presenter; // all props (xname, props) from within presenter
    ...
    const _____presenter = (_data_presenter && _data_presenter.props ? _data_presenter.props : undefined) || undefined; // TODO, should not support, keep it
