...
import { Filter } from "../gateway"; // import requireConfig from '@/components/AutoX/requireConfig';
...
function AutoLayout({
  ...
  const data = dataSource || rest || {}; // Cart

  const __cart = (cart && typeof cart === 'string' ? {
    xname: cart
  } : cart) || {};

  const _cart = sugarCart({ ...__cart,
    indicator: indicator,
    selector: selector
  });
