...
import { NamedContainer, NamedLayout, NamedGateway, NamedCart, NextIndicator, NamedIndicator } from "./..";
import DefaultContainer from "../container/Container";
import AutoPreviewIndicator from "../indicator/AutoPreviewIndicator";
import { useReplacing } from "../gateway/Replacing";
import { get as NamedPresenterGet } from "../config/NamedPresenterConfig";
import { get as DefaultIndicatorGet } from "../config/NamedIndicatorConfig";
...
function AutoLayout(autoLayoutProps) {
  const {
    ...
    allComponents = {},
    onItemClick = () => {
      console.log('AutoLayout-未设置onItemClick点击事件');
    },
    dataSource,
    onItemDeleted,
    onItemAdded,
  ...
  tagged(_tag, rest);
  const _dataSource = (Array.isArray(dataSource) ? {
    items: dataSource
  } : dataSource) || (Array.isArray(mock) ? {
    items: mock
  } : mock) || {};
  ...
  if (undefined && alternativeActive) {
    ...
    const alternative_layout = typeof notnull_alternative === 'string' ? {
      xname: notnull_alternative
    } : notnull_alternative.layout ? notnull_alternative.layout : notnull_alternative;
    ...
    // alternativeBack
    const {
      _Component: _AlternativeBack,
      _component: _alternativeBack
    } = getComponent(notnull_alternative.alternativeBack, DefaultIndicatorGet());
    return /*#__PURE__*/React.createElement(_AlternativeBack, _extends({}, _alternativeBack, {
      onBack: onAlternativeBack
    }), /*#__PURE__*/React.createElement(AutoLayout, _extends({}, layoutConfig, _dataSource, rest, alternativeOthers)));
  }
  const PreviewIndicator = ({
    children
  }) => {
    return /*#__PURE__*/React.createElement(NamedIndicator, {
  ...
  // Cart
  const _align_cart = (cart && typeof cart === 'string' ? {
    xname: cart
  } : cart) || undefined;
  const __cart = sugarCart({
    cart: _align_cart,
    indicator: indicator,
    selector: selector,
  ...
  const _1_gateway = _layoutGateway ? typeof _layoutGateway === 'string' ? {
    xname: _layoutGateway
  } : sugarGateway(_layoutGateway) : undefined;
  const _gateway = _1_gateway || (_layoutBinding ? {
    xname: "Binding"
  } : _layoutFilter ? {
    xname: "Filter"
  } : _layoutChain ? {
  ...
  const _presenter = Presenter && isJsonObject(presenter) ? {
    layout: {
      ...presenter,
      ...rest.___presenter
    }
  ...
  if (!Presenter && !layoutChildren && !container) {
    ...
    const nextTag = tag ? {
      tag: `${tag}-native-${_t_presenterName}`
    } : {};
    const _t_presenter = (_t_presenterName === 'autolayout' ? {
      layout: {
        ...___presenter.props,
        ...nextTag
      }
    } : ___presenter.props) || _props || tips(_props);
    ...
    return /*#__PURE__*/React.createElement(__NamedGateway, _extends({
      tag: `${_tag}-__NamedGateway__[${_gatewayName}]`,
      binding: _data_binding,
      filter: _data_filter,
      chain: _data_chain,
      gateway: _data_gateway
    }, _rest), /*#__PURE__*/React.createElement(__NamedCart, _extends({
      tag: `${_tag}-__NamedCart__`
    }, _data_cart), /*#__PURE__*/React.createElement(__Presenter, _extends({
      tag: `${_tag}-__Presenter__`
    }, _t_presenter, {
  ...
  return layoutChildren ? /*#__PURE__*/React.createElement(___previewIndicator, {
    xseq: xseq,
    onAutoPreview: onAutoPreview
  }, /*#__PURE__*/React.createElement(Container, _extends({}, _container, _dataSource, rest, {
    ...
    return /*#__PURE__*/React.createElement(_NamedGateway, {
      key: i,
      binding: _layoutBinding,
      filter: _layoutFilter,
      chain: _layoutChain,
      gateway: _gateway,
      tag: `${itemTag}-gateway[${_gatewayName}]`
    }, /*#__PURE__*/React.createElement(_NamedCart, _extends({}, __cart, {
      onItemDeleted: onItemDeleted,
      onItemAdded: onItemAdded,
      onItemChanged: onItemChanged,
      onItemIndicated: onItemIndicated,
      tag: `${_tag}-cart[${_cartName}]`
    }), /*#__PURE__*/React.createElement(__Presenter, _extends({
      ___: ___,
      xseq: xseq
    }, __presenter, {
      allComponents: allComponents,
  ...
  })))) : /*#__PURE__*/React.createElement(___previewIndicator, {
    xseq: xseq,
    onAutoPreview: onAutoPreview
  }, /*#__PURE__*/React.createElement(Container, _extends({}, _container, _dataSource, rest, {
    navigation: navigation,
  ...
  }, /*#__PURE__*/React.createElement(_NamedGateway, {
    binding: _layoutBinding,
    filter: _layoutFilter,
    chain: _layoutChain,
    gateway: _gateway,
    ...
    onItemIndicated: onItemIndicated,
    tag: `${_tag}-cart[${_cartName}]`
  }), presenter ? /*#__PURE__*/React.createElement(Presenter, _extends({
    ___: ___,
    xseq: xseq
  }, _presenter, {
    allComponents: allComponents,
