...
export default function Chain({
  ...
  return React.Children.toArray(children).map(child => /*#__PURE__*/React.cloneElement(child, { ...chaindata,
    ...rest
  }));
}
...
function doChain(chain, dataSource) {
  ...
  chain.map(item => {
    ...
    Object.keys(item).forEach(key => {
      const data = (Object.keys(chaindata).length > 0 ? chaindata : undefined) || dataSource; // if key == _, means get item
      ...
      if (key === '_' && Array.isArray(data)) {
        ...
        if (Array.isArray(keyValue) && keyValue.length == 0) {
          ...
          itemData = data[key]; // clean up chaindata
    ...
    chaindata = Array.isArray(chaindata) ? chaindata : { ...itemData,
      ...chaindata
    };
  });
  return { ...chaindata
