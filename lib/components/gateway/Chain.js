...
export default function Chain({
  ...
  const data = dataSource || rest || {}; // console.log('Chain.dataSource=', dataSource)
  // console.log('Chain.rest=', rest)

  const chaindata = chain.length > 0 ? doChain(chain, data) : {};
  const processedDataSource = dataSource ? {
    dataSource: chaindata
  } : chaindata;
  return React.Children.toArray(children).map(child => /*#__PURE__*/React.cloneElement(child, { ...processedDataSource,
    ...rest
  }));
}