import React from 'react'; // 需要测试跟以下的区别
...
export default function Filter({
  ...
  const filtereddata = typeof filter === 'string' ? data[filter] : filterData(data, filter); // if (Array.isArray(data)) {
  //   field = field[itemIndex];
  // }

  return React.Children.toArray(children).map(child => /*#__PURE__*/React.cloneElement(child, { ...filtereddata,
    ...rest
  }));
}

function filterData(data = {}, filter = {}) {
  let filterData = {}; // const filter = {
  //   name: 'title',
  //   cities: [],
  //   orders: {}
  //}
  ...
  Object.keys(filter).forEach(key => {
    const filterValue = filter[key]; // 1. the same as binding
