 ...
 * @returns 
 */
export default function BackIndicator(props) {

    const { children, __onIndicatorClick, onBack = __onIndicatorClick, ...rest } = props;
    // console.log('BackIndicator.__onIndicatorClick: ', __onIndicatorClick)

    const onClickBack = () => {
        if(onBack){
            onBack()
        }else{
            console.log('BackIndicator.onBack is not set -> just show called log.')
        }
    }
    ...
    return (
        <div>
            <Stack direction='row' justifyContent={'flex-start'}>
                 <div style={{ cursor: 'pointer', padding:'5px 10px'}}  onClick={onClickBack}>
