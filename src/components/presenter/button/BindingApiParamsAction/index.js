...
export default function DataetBindingAction(props){
    ...
    const datasetApi = `/api/auto/lc_low_auto_module_dataset/module_dataset/${datasetName}`;
    ...
    const datasetResp = useTokenRequest({ api:_datasetApi });
    const _datasetList = (datasetResp && datasetResp[0]) || {}

    const bindingResp = useTokenRequest({ api:_bindingApi });
    ...
    return (
        <>
            {
                !onRefresh && _datasetObj && JSON.stringify(_datasetObj) != '{}' ? (
                    Object.keys(_datasetObj).map((_key, _index) => (
                        <TreeItem key={_index} keyName={_key} 
            ...
            <ChildParamstListModal 
                isModalOpen={isModalOpen} onModalClose={modalClose}
                moduleName={moduleName}
                keyName={currentKeyName} keyValue={currentKeyValue}
                onCompleted={onCompleted}
            />
        </>
    )
