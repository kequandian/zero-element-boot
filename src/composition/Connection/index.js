...
import {
  ...
  PopoverAnchor,
  Portal, Grid, HStack,
  sm, closeOnEsc, Textarea, Tooltip, useToast 
            ...
            data.map((item, index) => {
              ...
              if (item.indexOf(".sql") > -1) {
                ...
                if (item.indexOf("ruler") > -1) {
                  ...
                  return <Container onClick={() => getshow(item, isDataSheetDetail)} maxW='container.xl' key={`${index}_item`}>
                    {!isDataSheetDetail?(
                      <div className='data-sheet-item'>{item}</div>
                    ):(
                      <div>{item}</div>
                    )}
                  </Container>
              ...
              <VStack spacing='3px'>
                ...
                <div> <Tabs variant='soft-rounded' colorScheme='green' >
                  <TabList style={{ position: 'absolute', left: '32% ' }}>
                    <Tab onClick={() => signWay(sign)}>数据表</Tab>
                    ...
                    : isShowData && showDetail ? (
                      <div style={{ width: '100%', paddingLeft: '50px' }}>
                        <Box flex='1'>
                          ...
                          ) : isShowData && dataSheetItemDetail.length > 0 ? (
                            <div style={{ background: '#ffffff', width: '100%', paddingTop: '10px' }}>
                              {handleContent(dataSheetItemDetail, true)}
                            </div>
                          ): <></>}
                        </Box>
                      </div>
