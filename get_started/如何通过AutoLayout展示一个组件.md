...
## ClickIndicator 点击事件处理经验

### 🎯 问题背景
在使用 `NamedIndicator` 包装 `ClickIndicator` 处理点击事件时，经常遇到点击事件无法正确触发的问题。

### 🔧 解决方案

#### 1. **正确的参数传递方式**
```javascript
// ✅ 正确：直接传递 onItemClick 给 NamedIndicator
<NamedIndicator
  indicator={{
...
// ❌ 错误：将 onItemClick 放在 indicator.props 中
<NamedIndicator
  indicator={{
    xname: 'ClickIndicator',
    props: {
      onItemClick: (indicatorData) => handleComponentClick(indicatorData.value)  // 不会被传递
    }
  }}
>
```
...
#### 2. **数据绑定机制**
`ClickIndicator` 通过 `indicatorData` 接收绑定数据：
```javascript
// NamedIndicator 中的 processIndicatorData 函数处理 binding
...
#### 3. **完整的点击事件流程**
```javascript
// 1. ClickIndicator 接收点击事件
function itemClick(){
  if(onItemClick){
    onItemClick(indicator)  // 传递 indicator 参数，结构为 {xname:'', props:{}}
  }
}

// 2. NamedIndicator 构造 indicator 参数结构
const indicatorParam = {
  xname: indicator.xname || 'ClickIndicator',
  props: processedIndicatorData  // binding 处理后的数据
}

// 3. 事件处理函数接收正确的数据
const handleComponentClick = (component) => {
  navigateToComponent(component);  // component 来自 indicator.props.value
};
```

### 📝 关键要点
1. **参数传递**：`onItemClick` 必须直接传递给 `NamedIndicator`，不能放在 `indicator.props` 中
2. **数据绑定**：使用 `binding` 配置数据映射，`dataSource` 提供数据源
3. **事件处理**：`ClickIndicator` 通过 `indicator` 参数传递数据，结构为 `{xname:'', props:{}}`
4. **组件层次**：确保 `NamedIndicator` → `ClickIndicator` → `onItemClick` 的数据流正确
5. **参数结构**：`indicator.props` 包含 binding 处理后的数据，如 `indicator.props.value`

### 🚨 常见错误
- 将 `onItemClick` 放在 `indicator.props` 中导致事件无法传递
- 忘记传递 `dataSource` 导致 `binding` 无法获取数据
- `indicator` 参数结构不正确导致事件处理函数接收错误参数
- 使用旧的 `indicatorData` 参数名而不是新的 `indicator` 参数名
