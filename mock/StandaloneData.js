export default {
  'GET /dev/dependency/json': {
    "code": 200,
    "message": "Success",
    "data": [
        "BOOT-INF/classes/application.yml",
        "BOOT-INF/classes/com/jfeat/AmApplication.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/api/IndexingJarDeployEndpoint.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/api/JarDeployEndpoint$1.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/api/JarDeployEndpoint.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/properties/JarDeployProperties.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/request/JarRequest.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/util/DepUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/util/IndexingUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dep/util/UploadUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/DecompileUtils$1.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/DecompileUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/DependencyUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/FileUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/JarFileUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/JarUpdate.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/ZipFileUtils.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/api/DependencyEndpoint.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/cli/MainMethod.class",
        "BOOT-INF/classes/com/jfeat/jar/dependency/comparable/ChecksumKeyValue.class",
        "BOOT-INF/classes/resources.properties",
        "BOOT-INF/lib/aspectjweaver-1.8.10.jar",
        "BOOT-INF/lib/byte-buddy-1.8.12.jar",
        "BOOT-INF/lib/cfr-0.151.jar",
        "BOOT-INF/lib/classmate-1.3.3.jar",
        "BOOT-INF/lib/commons-cli-1.3.1.jar",
        "BOOT-INF/lib/commons-io-2.5.jar",
        "BOOT-INF/lib/commons-lang3-3.9.jar",
        "BOOT-INF/lib/crud-base-0.0.1.jar",
        "BOOT-INF/lib/dev-cors-0.0.1.jar",
        "BOOT-INF/lib/dev-dependency-0.0.1.jar",
        "BOOT-INF/lib/fastjson-1.2.73.jar",
        "BOOT-INF/lib/guava-20.0.jar",
        "BOOT-INF/lib/h2-1.4.200.jar",
        "BOOT-INF/lib/hibernate-validator-5.3.5.Final.jar",
        "BOOT-INF/lib/jackson-annotations-2.8.0.jar",
        "BOOT-INF/lib/jackson-core-2.8.9.jar",
        "BOOT-INF/lib/jackson-databind-2.8.9.jar",
        "BOOT-INF/lib/javax.el-3.0.0.jar",
        "BOOT-INF/lib/javax.servlet-api-3.1.0.jar",
        "BOOT-INF/lib/jboss-annotations-api_1.2_spec-1.0.0.Final.jar",
        "BOOT-INF/lib/jboss-logging-3.3.1.Final.jar",
        "BOOT-INF/lib/jboss-websocket-api_1.1_spec-1.1.0.Final.jar",
        "BOOT-INF/lib/jcl-over-slf4j-1.7.25.jar",
        "BOOT-INF/lib/jul-to-slf4j-1.7.25.jar",
        "BOOT-INF/lib/log4j-over-slf4j-1.7.25.jar",
        "BOOT-INF/lib/logback-classic-1.1.11.jar",
        "BOOT-INF/lib/logback-core-1.1.11.jar",
        "BOOT-INF/lib/lombok-1.18.8.jar",
        "BOOT-INF/lib/mapstruct-1.2.0.Final.jar",
        "BOOT-INF/lib/maven-model-3.6.3.jar",
        "BOOT-INF/lib/plexus-utils-3.2.1.jar",
        "BOOT-INF/lib/slf4j-api-1.7.25.jar",
        "BOOT-INF/lib/snakeyaml-1.17.jar",
        "BOOT-INF/lib/spring-aop-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/spring-beans-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-autoconfigure-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-starter-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-starter-aop-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-starter-logging-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-starter-undertow-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-boot-starter-web-1.5.6.RELEASE.jar",
        "BOOT-INF/lib/spring-context-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/spring-core-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/spring-expression-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/spring-plugin-core-1.2.0.RELEASE.jar",
        "BOOT-INF/lib/spring-plugin-metadata-1.2.0.RELEASE.jar",
        "BOOT-INF/lib/spring-web-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/spring-webmvc-4.3.10.RELEASE.jar",
        "BOOT-INF/lib/springfox-core-2.9.2.jar",
        "BOOT-INF/lib/springfox-schema-2.9.2.jar",
        "BOOT-INF/lib/springfox-spi-2.9.2.jar",
        "BOOT-INF/lib/springfox-spring-web-2.9.2.jar",
        "BOOT-INF/lib/springfox-swagger-common-2.9.2.jar",
        "BOOT-INF/lib/springfox-swagger-ui-2.9.2.jar",
        "BOOT-INF/lib/springfox-swagger2-2.9.2.jar",
        "BOOT-INF/lib/swagger-annotations-1.5.20.jar",
        "BOOT-INF/lib/swagger-models-1.5.20.jar",
        "BOOT-INF/lib/undertow-core-1.4.18.Final.jar",
        "BOOT-INF/lib/undertow-servlet-1.4.18.Final.jar",
        "BOOT-INF/lib/undertow-websockets-jsr-1.4.18.Final.jar",
        "BOOT-INF/lib/validation-api-1.1.0.Final.jar",
        "BOOT-INF/lib/xnio-api-3.3.8.Final.jar",
        "BOOT-INF/lib/xnio-nio-3.3.8.Final.jar",
        "META-INF/maven/com.jfeat/dev-dependency-ui/pom.properties",
        "META-INF/maven/com.jfeat/dev-dependency-ui/pom.xml",
        "org/springframework/boot/loader/ExecutableArchiveLauncher$1.class",
        "org/springframework/boot/loader/ExecutableArchiveLauncher.class",
        "org/springframework/boot/loader/JarLauncher.class",
        "org/springframework/boot/loader/LaunchedURLClassLoader$1.class",
        "org/springframework/boot/loader/LaunchedURLClassLoader.class",
        "org/springframework/boot/loader/Launcher.class",
        "org/springframework/boot/loader/MainMethodRunner.class",
        "org/springframework/boot/loader/PropertiesLauncher$1.class",
        "org/springframework/boot/loader/PropertiesLauncher$ArchiveEntryFilter.class",
        "org/springframework/boot/loader/PropertiesLauncher$PrefixMatchingArchiveFilter.class",
        "org/springframework/boot/loader/PropertiesLauncher.class",
        "org/springframework/boot/loader/WarLauncher.class",
        "org/springframework/boot/loader/archive/Archive$Entry.class",
        "org/springframework/boot/loader/archive/Archive$EntryFilter.class",
        "org/springframework/boot/loader/archive/Archive.class",
        "org/springframework/boot/loader/archive/ExplodedArchive$1.class",
        "org/springframework/boot/loader/archive/ExplodedArchive$FileEntry.class",
        "org/springframework/boot/loader/archive/ExplodedArchive$FileEntryIterator$EntryComparator.class",
        "org/springframework/boot/loader/archive/ExplodedArchive$FileEntryIterator.class",
        "org/springframework/boot/loader/archive/ExplodedArchive.class",
        "org/springframework/boot/loader/archive/JarFileArchive$EntryIterator.class",
        "org/springframework/boot/loader/archive/JarFileArchive$JarFileEntry.class",
        "org/springframework/boot/loader/archive/JarFileArchive.class",
        "org/springframework/boot/loader/data/ByteArrayRandomAccessData.class",
        "org/springframework/boot/loader/data/RandomAccessData$ResourceAccess.class",
        "org/springframework/boot/loader/data/RandomAccessData.class",
        "org/springframework/boot/loader/data/RandomAccessDataFile$DataInputStream.class",
        "org/springframework/boot/loader/data/RandomAccessDataFile$FilePool.class",
        "org/springframework/boot/loader/data/RandomAccessDataFile.class",
        "org/springframework/boot/loader/jar/AsciiBytes.class",
        "org/springframework/boot/loader/jar/Bytes.class",
        "org/springframework/boot/loader/jar/CentralDirectoryEndRecord.class",
        "org/springframework/boot/loader/jar/CentralDirectoryFileHeader.class",
        "org/springframework/boot/loader/jar/CentralDirectoryParser.class",
        "org/springframework/boot/loader/jar/CentralDirectoryVisitor.class",
        "org/springframework/boot/loader/jar/FileHeader.class",
        "org/springframework/boot/loader/jar/Handler.class",
        "org/springframework/boot/loader/jar/JarEntry.class",
        "org/springframework/boot/loader/jar/JarEntryFilter.class",
        "org/springframework/boot/loader/jar/JarFile$1.class",
        "org/springframework/boot/loader/jar/JarFile$2.class",
        "org/springframework/boot/loader/jar/JarFile$3.class",
        "org/springframework/boot/loader/jar/JarFile$JarFileType.class",
        "org/springframework/boot/loader/jar/JarFile.class",
        "org/springframework/boot/loader/jar/JarFileEntries$1.class",
        "org/springframework/boot/loader/jar/JarFileEntries$EntryIterator.class",
        "org/springframework/boot/loader/jar/JarFileEntries.class",
        "org/springframework/boot/loader/jar/JarURLConnection$1.class",
        "org/springframework/boot/loader/jar/JarURLConnection$JarEntryName.class",
        "org/springframework/boot/loader/jar/JarURLConnection.class",
        "org/springframework/boot/loader/jar/ZipInflaterInputStream.class",
        "org/springframework/boot/loader/util/SystemPropertyUtils.class"
    ]
  }
}
